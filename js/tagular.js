(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["tagular"] = factory();
	else
		root["tagular"] = factory();
})(window, function() {
...
Object.defineProperty(exports, "__esModule", { value: true });
var tag_1 = __webpack_require__(/*! ./tag/tag */ "./src/tag/tag.ts");
var reader_1 = __webpack_require__(/*! ./reader/reader */ "./src/reader/reader.ts");
console.log("**********************************");
...
console.log("**********************************");
exports.default = { tag: tag_1.tag, reader: reader_1.default };
...
Object.defineProperty(exports, "__esModule", { value: true });
var tag_1 = __webpack_require__(/*! ../tag/tag */ "./src/tag/tag.ts");
function recursive(tag, targetComment) {
    if (targetComment === void 0) { targetComment = null; }
    tag.punchingHole.forEach(function (d) {
        if (d.value instanceof Array) {
            d.value.forEach(function (f) {
                if (f instanceof tag_1.TAG) {
    ...
    if (targetComment) {
        while (tag.fragment.children[0].childNodes.length) {
            var t = Array.from(tag.fragment.children[0].childNodes).shift();
...
function textDiff(oph, nph) {
    var ophValues = __spreadArrays(oph.value);
    var nphValues = __spreadArrays(nph.value);
    var ophTargets = __spreadArrays(oph.target);
    var idx = 0;
    while (ophValues.length && nphValues.length) {
        var ophValue = ophValues.shift();
        var nphValue = nphValues.shift();
        var ophTarget = ophTargets.shift();
...
function diff(oldTag, newTag, OPH, NPH) {
    if (OPH === void 0) { OPH = null; }
    if (NPH === void 0) { NPH = null; }
    // punching text 비교
    var oldTagString = oldTag.punchingText.map(function (d) { return d.trim(); }).join("");
    var newTagString = newTag.punchingText.map(function (d) { return d.trim(); }).join("");
    if ((oldTagString.length === newTagString.length)) {
        var oldPunchingHole = __spreadArrays(oldTag.punchingHole);
        var newPunchingHole = __spreadArrays(newTag.punchingHole);
        var _loop_1 = function () {
            var oph = oldPunchingHole.shift();
            var nph = newPunchingHole.shift();
            if ((oph.type === "text") && (nph.type === "text")) {
                if (oph.value.length === nph.value.length) {
                    ...
                    for (var i = 0; i < lengthDiff; i++) {
                        var node = document.createTextNode(nph.value[(nph.value.length - 1) - i]);
                ...
                oph.type = "text";
                nph.value.forEach(function (d) {
                    var node = document.createTextNode(nph.value);
                ...
                oph.target.forEach(function (t) { return t.remove(); });
                oph.type = "undefined";
                oph.target = [];
                oph.value = [];
            }
            else if ((oph.type === "text") && (nph.type === "hasChild")) {
                oph.target.forEach(function (d) {
                    d.remove();
                });
                oph.type = "hasChild";
                oph.target = [];
                nph.value.forEach(function (d) {
                    ...
                    while (d.fragment.children[0].childNodes.length) {
                        var t = Array.from(d.fragment.children[0].childNodes).shift();
        ...
        while (newTag.fragment.children[0].childNodes.length) {
            var t = Array.from(newTag.fragment.children[0].childNodes).shift();
            if (t instanceof Text) {
                if ((/^(?!.)/s.test(t.textContent.trim()))) {
                    t.remove();
                    continue;
...
function initTarget(oldTag) {
    var oldTagString = oldTag.tagText.map(function (d) { return d.trim(); }).join("");
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.TAG = exports.tag = void 0;
...
var TAG = /** @class */ (function () {
    function TAG(strings) {
        ...
        this.punchingText = strings.map(function (d) { return d.replace(/\s+/g, ' ').trim(); });
        this.punchingHole = __spreadArrays(expr);
    ...
    TAG.prototype.clone = function () {
        return new (TAG.bind.apply(TAG, __spreadArrays([void 0, this.punchingText], this.punchingHole.map(function (d) { return d.value; }))))();
    };
    TAG.prototype.isTagArray = function (array) {
        var flat = false;
    ...
    TAG.prototype.fillHole = function () {
        var expr = __spreadArrays(this.punchingHole);
        var txt = __spreadArrays(this.punchingText);
        while (expr.length) {
            var e = expr.shift();
            if (e instanceof Array) {
                var flag = this.isTagArray(e);
                if (flag) {
    ...
    TAG.prototype.createFragment = function () {
    ...
    TAG.prototype.dfs = function (root, expr) {
        ...
        var _loop_1 = function () {
            ...
            if (curr.hasChildNodes())
                list.unshift.apply(list, curr.childNodes);
            if (curr.attributes && curr.attributes.length) {
                var attr = curr.attributes;
                for (var i = 0; i < curr.attributes.length; i++) {
                    var currentAttr = attr[i];
            ...
            if (curr instanceof Comment) {
                if (curr.textContent === this_1.id) {
                    var v = this_1.punchingHole.shift();
                    if (v !== undefined) {
                ...
                else if (curr.textContent === "T" + this_1.id) {
                    var v = this_1.punchingHole.shift();
                    if (v !== undefined) {
                        if (v instanceof Array) {
                            var info_2 = {
...
function tag(strings) {
    ...
    return new (TAG.bind.apply(TAG, __spreadArrays([void 0, strings], expr)))();
}
exports.tag = tag;
...
//# sourceMappingURL=tagular.js.map